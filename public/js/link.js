...
(() => {
    ...
    // ==================== DELETE CONFIRMATION ====================
    // Fungsi ini untuk menghapus LINK, bukan kategori
    ...
    const initChart = visitData => {
        ...
        const config = {
            ...
            chart: { type: 'area', height: 320, toolbar: { show: true }, animations: { enabled: true }, zoom: { enabled: false } },
            stroke: { width: 3, curve: 'smooth' },
            markers: { size: 5, colors: ['#fff'], strokeColors: ['#007bff', '#ff6b6b'], strokeWidth: 2, hover: { size: 7 } },
            grid: { borderColor: 'rgba(0,0,0,0.1)', strokeDashArray: 3, xaxis: { lines: { show: false } }, yaxis: { lines: { show: true } } },
            xaxis: { categories: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'], axisTicks: { show: false }, axisBorder: { show: false } },
            yaxis: { tickAmount: 5, labels: { formatter: val => `${val} Visit` } },
            tooltip: { theme: 'dark', x: { show: true }, y: { formatter: val => `${val} Visit` } },
            colors: ['#007bff', '#ff6b6b'],
            fill: { type: 'gradient', gradient: { shade: 'light', type: 'vertical', shadeIntensity: 0.25, gradientToColors: ['#80d0ff', '#ff9f9f'], opacityFrom: 1, opacityTo: 0.3, } },
    ...
    /**
     * Menangani konfirmasi hapus untuk form KATEGORI.
     */
    const setupDeleteCategoryForms = () => {
        ...
        deleteForms.forEach(form => {
            form.addEventListener('submit', function (event) {
                ...
                // Dapatkan nama kategori dari elemen lain untuk pesan yang lebih dinamis
                const categoryName = form.closest('li').querySelector('a.nav-link').textContent.trim();
                    ...
                    if (result.isConfirmed) {
                        ...
                        .catch(error => {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Gagal',
                                text: error.message || 'Gagal memperbarui status kategori.',
                            });
                        });
                    }
                });
